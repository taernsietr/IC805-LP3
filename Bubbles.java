...
public class Bubbles extends PApplet {
  ...
  public void draw() {
    ...
    for(int i = 0; i < particles.size(); i++) {
      particles.get(i).force(new PVector(rand.nextFloat(-0.01f, 0.01f) * particles.get(i).mass, 
                                         rand.nextFloat(-0.01f, 0.01f) * particles.get(i).mass));
      
      particles.get(i).update();
      clipParticle(particles.get(i));

      fill(map(particles.get(i).size, 0.01f, 1.0f, 0, 63),
